...
//배열, 셋, 또는 딕셔너리를 생성하고, 이를 변수에 할당하면, 생성된 ‘집합체 (collection)’ 는 변경 가능 (mutable) 할 것입니다.
//이는 생성 후에 이 집합체에 항목을 추가, 삭제, 또는 바꿈으로써 집합체를 바꿀-또는 변경할 (mutate)-수 있다는 의미입니다. 배열, 셋, 또는 딕셔너리를 상수에 할당하면, 해당 집합체는 변경 불가능 (immutable) 이며, 그 크기와 내용을 바꿀 수 수 없습니다.
