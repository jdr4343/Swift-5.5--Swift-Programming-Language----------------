...
//(‘셋’ 으로) 기본적인 ‘집합 연산 (set operations)’ 들, 가령 두 집합을 서로 ‘조합하는 것 (combining)’,
//두 집합이 공통적으로 가지고 있는 값을 결정하는 것, 또는 두 집합이 모두 같은, 일부만 같은, 아니면 아무 것도 같지 않은 값들을 담고 있는 지를 결정하는 것 등, 을 효율적으로 수행할 수 있습니다.
